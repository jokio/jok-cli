...
export function exampleHelp() {
  return `Example from https://github.com/jokio/jok-cli/tree/master/examples/ ${output.param(
    'example-path',
  )}`
}
...
export function copying(projectName, templateName) {
  return `
Creating ${chalk.bold(chalk.green(projectName))} ${chalk.gray(
